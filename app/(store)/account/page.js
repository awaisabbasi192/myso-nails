...
export default async function AccountPage() {
  ...
  const [ordersRaw, addresses, products, customer] = await Promise.all([
    ...
    prisma.customer.findUnique({ where: { id: user.id }, select: { nailSizes: true, createdAt: true } }),
  ]);
  ...
  return (
    <AccountClient
      user={{ id: user.id, name: user.name, email: user.email, phone: user.phone, nailSizes: customer?.nailSizes || "", memberSince: fmtDate(customer?.createdAt || new Date()) }}
      orders={orders}
