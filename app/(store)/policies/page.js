...
const SECTIONS = [
  {
    ...
    points: [
      ...
      "Never send payment to any number other than the one shown at checkout. Press-Ons by Myra is not responsible for money sent to incorrect accounts.",
    ],
  },
  {
    id: "delivery",
...
export default function PoliciesPage() {
  return (
    <div style={{ maxWidth: 860, margin: "0 auto", padding: "56px 24px 100px" }}>
      <div style={{ fontSize: 10.5, letterSpacing: ".3em", textTransform: "uppercase", color: "rgba(168,121,104,.75)", marginBottom: 12 }}>Legal</div>
      ...
      <p style={{ fontSize: 15, lineHeight: 1.9, color: "rgba(247,241,237,.55)", fontWeight: 300, marginBottom: 20 }}>
        ...
        <a href={waLink("Hi Press-Ons by Myra! I have a question about your policies.")} target="_blank" rel="noreferrer" style={{ color: "var(--rose)", borderBottom: "1px solid rgba(168,121,104,.4)" }}>WhatsApp</a>.
      </p>

      {/* Quick nav */}
      <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 44, paddingBottom: 30, borderBottom: "1px solid rgba(168,121,104,.14)" }}>
        {SECTIONS.map((s) => (
          <a key={s.id} href={`#${s.id}`} style={{ fontSize: 10.5, letterSpacing: ".14em", textTransform: "uppercase", padding: "8px 14px", border: "1px solid rgba(168,121,104,.25)", color: "rgba(247,241,237,.65)" }}>{s.title}</a>
        ))}
      </div>

      {/* Highlight box */}
      <div style={{ border: "1px solid rgba(168,121,104,.3)", background: "var(--panel)", padding: "24px 28px", marginBottom: 44, display: "flex", flexDirection: "column", gap: 10 }}>
      ...
      <div style={{ borderTop: "1px solid rgba(168,121,104,.14)", paddingTop: 30, marginTop: 20, display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 16, alignItems: "center" }}>
        <div style={{ fontSize: 12, color: "rgba(247,241,237,.4)" }}>Last updated: July 2026 · Press-Ons by Myra, Lahore</div>
