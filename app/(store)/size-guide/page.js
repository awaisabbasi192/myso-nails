...
export default function SizeGuidePage() {
  ...
  return (
    <div style={{ maxWidth: 1100, margin: "0 auto", padding: "60px 24px 110px" }}>
      <div style={{ fontSize: 11, letterSpacing: ".4em", textTransform: "uppercase", color: "rgba(168,121,104,.75)", marginBottom: 14 }}>Sizing</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: 14, marginBottom: 60 }}>
        {sizes.map((s) => (
          <div key={s.finger} style={{ border: "1px solid rgba(168,121,104,.18)", background: "var(--panel)", padding: 24 }}>
      ...
      {/* MM ruler visual */}
      <div style={{ border: "1px solid rgba(168,121,104,.2)", background: "var(--panel)", padding: "26px 30px", marginBottom: 52 }}>
        ...
        <div style={{ display: "flex", gap: 0, alignItems: "flex-end", overflowX: "auto", paddingBottom: 8 }}>
          {Array.from({ length: 21 }, (_, i) => i + 10).map((mm) => (
            <div key={mm} style={{ display: "flex", flexDirection: "column", alignItems: "center", minWidth: 42, flexShrink: 0 }}>
              <div style={{ height: mm % 5 === 0 ? 22 : 12, width: 1, background: mm % 5 === 0 ? "var(--bronze)" : "rgba(168,121,104,.3)" }} />
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(220px,1fr))", gap: 18, marginBottom: 52 }}>
        {steps.map((s) => (
          <div key={s.n} style={{ borderTop: "1px solid rgba(168,121,104,.25)", paddingTop: 20 }}>
      ...
      {/* Free sizing kit */}
      <div className="kit-cta" style={{ border: "1px solid rgba(168,121,104,.22)", background: "var(--panel)", padding: 34, display: "grid", gridTemplateColumns: "1fr auto", gap: 24, alignItems: "center" }}>
        <div>
        ...
        <a href={waLink("Hi Myra! I'd like a free sizing kit please")} target="_blank" rel="noreferrer" className="btn-wa" style={{ padding: "15px 24px", fontSize: 11, letterSpacing: ".2em", textTransform: "uppercase", whiteSpace: "nowrap" }}>Request on WhatsApp</a>
      </div>
    </div>
  );
}
