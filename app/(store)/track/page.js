...
export default function TrackPage() {
  ...
  const inp = { background: "transparent", border: "1px solid rgba(168,121,104,.25)", color: "#F7F1ED", padding: 14, fontSize: 13, outline: "none", width: "100%" };

  return (
    <div style={{ maxWidth: 680, margin: "0 auto", padding: "64px 24px 110px" }}>
      <div style={{ fontSize: 11, letterSpacing: ".4em", textTransform: "uppercase", color: "rgba(168,121,104,.75)", marginBottom: 14 }}>Order tracking</div>
      ...
      <div style={{ border: "1px solid rgba(168,121,104,.18)", background: "var(--panel)", padding: 30 }}>
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          {error && <div style={{ fontSize: 12.5, color: "#E0A099", borderLeft: "2px solid rgba(200,90,90,.5)", paddingLeft: 12 }}>{error}</div>}
      ...
      {result && (
        <div style={{ marginTop: 28, border: "1px solid rgba(168,121,104,.2)", background: "var(--panel)", padding: 28 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 22, flexWrap: "wrap", gap: 12 }}>
            ...
            <div style={{ fontSize: 10, letterSpacing: ".16em", textTransform: "uppercase", border: `1px solid ${result.status === "Delivered" ? "rgba(143,214,166,.5)" : result.status === "Rejected" ? "rgba(200,90,90,.5)" : "rgba(168,121,104,.35)"}`, padding: "8px 14px", color: result.status === "Delivered" ? "#8FD6A6" : result.status === "Rejected" ? "#E0A099" : "var(--rose-light)" }}>{result.status}</div>
          </div>
          {result.status !== "Rejected" && (
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 0 }}>
              {STAGES.map((s, i) => {
                ...
                return (
                  <div key={s} style={{ borderTop: `2px solid ${done ? "var(--bronze)" : "rgba(168,121,104,.15)"}`, paddingTop: 12 }}>
          ...
          {result.status === "Rejected" && <div style={{ fontSize: 13, color: "#E0A099", marginTop: 8 }}>This order was not processed. Please contact us on WhatsApp.</div>}
          <div style={{ marginTop: 22, paddingTop: 18, borderTop: "1px solid rgba(168,121,104,.12)", display: "flex", gap: 12, flexWrap: "wrap" }}>
            <Link href="/shop" style={{ fontSize: 10.5, letterSpacing: ".2em", textTransform: "uppercase", border: "1px solid rgba(168,121,104,.3)", padding: "10px 18px", color: "rgba(247,241,237,.7)" }}>Shop more</Link>
          </div>
        </div>
      )}
    </div>
