...
const VALUES = [
  ...
  { title: "Real people on WhatsApp", body: "Myra answers the number herself. Sizing questions, quotes and order updates in the same chat." },
];
...
export default async function AboutPage() {
  ...
  const aboutBody1     = content?.aboutBody1     || "Press-Ons by Myra started in 2022 as a side table in Myra's bedroom in Lahore.";
  ...
  const aboutArtistName= content?.aboutArtistName|| "Myra, founder";
  ...
  const aboutArtistSign= content?.aboutArtistSign|| "Myra";
    ...
    <div>
      <section data-r="split" style={{ maxWidth: 1240, margin: "0 auto", padding: "60px 24px 30px", display: "grid", gridTemplateColumns: "1.05fr .95fr", gap: 56, alignItems: "center" }}>
        ...
        <div style={{ position: "relative" }}>
          ...
          <div style={{ position: "absolute", inset: -14, border: "1px solid rgba(168,121,104,.22)", pointerEvents: "none" }} />
        </div>
      </section>
      ...
      <section style={{ maxWidth: 1240, margin: "0 auto", padding: "50px 24px" }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: 20 }}>
          {STATS.map((s) => (
            <div key={s.label} style={{ borderTop: "1px solid rgba(168,121,104,.25)", paddingTop: 20 }}>
      ...
      <section style={{ background: "var(--panel)", borderTop: "1px solid rgba(168,121,104,.12)", borderBottom: "1px solid rgba(168,121,104,.12)", padding: "80px 24px" }}>
        <div style={{ maxWidth: 1240, margin: "0 auto" }}>
          <h2 style={{ fontFamily: "var(--serif)", fontWeight: 300, fontSize: 40, margin: "0 0 40px" }}>What we promise</h2>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(250px,1fr))", gap: 22 }}>
            {VALUES.map((v) => (
              <div key={v.title} style={{ border: "1px solid rgba(168,121,104,.16)", padding: 28, background: "var(--bg)" }}>
      ...
      <section data-r="split" style={{ maxWidth: 1240, margin: "0 auto", padding: "80px 24px", display: "grid", gridTemplateColumns: ".9fr 1.1fr", gap: 56, alignItems: "center" }}>
        ...
        <div>
          ...
          <div style={{ display: "flex", gap: 12, marginTop: 22, flexWrap: "wrap" }}>
            ...
            <Link href="/shop" style={{ border: "1px solid rgba(168,121,104,.35)", padding: "16px 28px", fontSize: 11, letterSpacing: ".24em", textTransform: "uppercase", color: "rgba(247,241,237,.8)" }}>Shop the sets</Link>
          </div>
        </div>
      </section>
    </div>
