...
const inp = { background: "transparent", border: "1px solid rgba(168,121,104,.25)", color: "var(--ink)", padding: "11px 14px", fontSize: 13, outline: "none", width: "100%" };
...
export default function AccountClient({ user: initUser, orders: initOrders, addresses: initAddresses, products }) {
  ...
  const TIERS = [
    { name: "Bronze", min: 0, perk: "Welcome to Press-Ons by Myra" },
  ...
  async function cancelOrder(order) {
    ...
    } else showToast(data.error || "Could not cancel order", "error");
  }
  ...
  async function saveAddr() {
    ...
    try {
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok) { showToast(data.error || "Could not save address", "error"); return; }
      showToast(isEdit ? "Address updated" : "Address saved", "success", "📍");
  ...
  return (
    <div style={{ maxWidth: 1150, margin: "0 auto", padding: "44px 24px 100px" }}>
      <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: 20, flexWrap: "wrap", marginBottom: 34 }}>
        <div>
          <div style={{ fontSize: 10.5, letterSpacing: ".3em", textTransform: "uppercase", color: "rgba(168,121,104,.75)" }}>My account</div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 18, flexWrap: "wrap" }}>
          ...
          <div onClick={logout} style={{ cursor: "pointer", fontSize: 10, letterSpacing: ".2em", textTransform: "uppercase", border: "1px solid rgba(168,121,104,.25)", padding: "9px 14px", color: "rgba(247,241,237,.7)" }}>Sign out</div>
        </div>
      </div>

      <div style={{ display: "flex", gap: 26, borderBottom: "1px solid rgba(168,121,104,.14)", marginBottom: 34, flexWrap: "wrap" }}>
        {tabs.map((t) => (
      ...
      {tab === "Overview" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 22 }}>
          {/* Active order banner */}
          {inProgressOrder && (
            <div onClick={() => setOrderDetail(inProgressOrder)} style={{ cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16, flexWrap: "wrap", border: "1px solid rgba(168,121,104,.35)", background: "linear-gradient(100deg,rgba(184,122,98,.14),rgba(242,205,187,.06))", padding: "18px 24px" }}>
              ...
              <div style={{ fontSize: 10, letterSpacing: ".2em", textTransform: "uppercase", border: "1px solid rgba(168,121,104,.4)", padding: "10px 16px", color: "var(--rose-light)" }}>Track →</div>
            </div>
          )}
          ...
          <div className="acct-stats" style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14 }}>
            {[
              ...
              <div key={s.label} style={{ border: "1px solid rgba(168,121,104,.16)", background: "var(--panel)", padding: "20px 18px" }}>
          ...
          {/* Loyalty tier */}
          <div style={{ border: "1px solid rgba(168,121,104,.2)", background: "var(--panel)", padding: "26px 28px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", flexWrap: "wrap", gap: 10 }}>
              <div>
                <div style={{ fontSize: 10, letterSpacing: ".24em", textTransform: "uppercase", color: "var(--rose)" }}>Myra rewards</div>
          ...
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 14 }}>
              ...
              {orders.length > 2 && <div onClick={() => setTab("Orders")} style={{ cursor: "pointer", fontSize: 11, color: "var(--rose)", borderBottom: "1px solid rgba(168,121,104,.4)" }}>View all</div>}
            </div>
            {orders.length === 0 ? (
              <div style={{ border: "1px dashed rgba(168,121,104,.25)", padding: 34, textAlign: "center", color: "rgba(247,241,237,.5)", fontSize: 13.5 }}>No orders yet. <Link href="/shop" style={{ color: "var(--rose)" }}>Start shopping</Link></div>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {orders.slice(0, 2).map((o) => (
                  <div key={o.id} onClick={() => setOrderDetail(o)} style={{ cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 14, border: "1px solid rgba(168,121,104,.14)", background: "var(--panel)", padding: "14px 18px" }}>
                    ...
                    <div style={{ fontSize: 10, letterSpacing: ".18em", textTransform: "uppercase", border: "1px solid rgba(168,121,104,.3)", padding: "7px 11px", color: o.status === "Cancelled" || o.status === "Rejected" ? "#E0A099" : "var(--rose)", whiteSpace: "nowrap", flexShrink: 0 }}>{o.status}</div>
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="acct-quick" style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12 }}>
            {[
              ...
              <Link key={q.href} href={q.href} style={{ border: "1px solid rgba(168,121,104,.16)", background: "var(--panel)", padding: "20px 14px", textAlign: "center", display: "flex", flexDirection: "column", gap: 8, alignItems: "center" }}>
      ...
      {tab === "Orders" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
          {orders.length === 0 && <div style={{ border: "1px dashed rgba(168,121,104,.25)", padding: 40, textAlign: "center", color: "rgba(247,241,237,.5)", fontSize: 13.5 }}>No orders yet. <Link href="/shop" style={{ color: "var(--rose)" }}>Start shopping</Link></div>}
          {orders.map((o) => {
            const stage = STAGES.indexOf(o.status);
            const rejected = o.status === "Rejected" || o.status === "Cancelled";
            return (
              <div key={o.id} style={{ border: "1px solid rgba(168,121,104,.16)", background: "var(--panel)", padding: 26 }}>
                <div style={{ display: "flex", justifyContent: "space-between", gap: 20, flexWrap: "wrap", alignItems: "center" }}>
                  ...
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                    <div style={{ fontSize: 10, letterSpacing: ".2em", textTransform: "uppercase", border: "1px solid rgba(168,121,104,.3)", padding: "9px 13px", color: rejected ? "#E0A099" : "var(--rose)", whiteSpace: "nowrap" }}>{o.status}</div>
                    <div onClick={() => setOrderDetail(o)} style={{ cursor: "pointer", fontSize: 10, letterSpacing: ".2em", textTransform: "uppercase", background: "rgba(168,121,104,.1)", border: "1px solid rgba(168,121,104,.3)", padding: "9px 13px" }}>Details</div>
                    <div onClick={() => { o.items.forEach((it) => it.slug && addToCart({ slug: it.slug, name: it.name, image: it.image, price: it.unitPrice, size: it.size }, it.qty)); router.push("/cart"); }} style={{ cursor: "pointer", fontSize: 10, letterSpacing: ".2em", textTransform: "uppercase", background: "rgba(168,121,104,.12)", border: "1px solid rgba(168,121,104,.3)", padding: "9px 13px" }}>Reorder</div>
                    {o.status === "Pending" && <div onClick={() => cancelOrder(o)} style={{ cursor: "pointer", fontSize: 10, letterSpacing: ".2em", textTransform: "uppercase", border: "1px solid rgba(200,90,90,.4)", color: "#E0A099", padding: "9px 13px" }}>Cancel</div>}
                ...
                {!rejected && (
                  <div data-r="steps4" style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 0, marginTop: 26 }}>
                    {STAGES.map((label, i) => (
                      <div key={label} style={{ borderTop: `2px solid ${i <= stage ? "var(--bronze)" : "rgba(168,121,104,.15)"}`, paddingTop: 12 }}>
      ...
      {tab === "Wishlist" && (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(230px,1fr))", gap: 20 }}>
          {wishProducts.length === 0 && <div style={{ gridColumn: "1/-1", border: "1px dashed rgba(168,121,104,.25)", padding: 40, textAlign: "center", color: "rgba(247,241,237,.5)", fontSize: 13.5 }}>Your wishlist is empty. <Link href="/shop" style={{ color: "var(--rose)" }}>Find something you love</Link></div>}
          {wishProducts.map((p) => (
            <div key={p.slug} className="card" style={{ background: "var(--panel)" }}>
              <Link href={`/product/${p.slug}`} style={{ display: "block", aspectRatio: "1/1", overflow: "hidden", position: "relative" }}>
                ...
                <div onClick={(e) => { e.preventDefault(); toggleWish(p.slug); }} style={{ position: "absolute", top: 12, right: 12, width: 34, height: 34, display: "grid", placeItems: "center", borderRadius: "50%", background: "rgba(10,10,11,.75)", border: "1px solid rgba(168,121,104,.28)", color: "var(--rose)" }}>♥</div>
              </Link>
              <div style={{ padding: 16, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span style={{ fontFamily: "var(--serif)", fontSize: 19 }}>{p.name}</span>
                <div onClick={() => addToCart({ slug: p.slug, name: p.name, image: p.image, price: p.price, size: "Medium set" }, 1)} style={{ cursor: "pointer", fontSize: 10, letterSpacing: ".18em", textTransform: "uppercase", border: "1px solid rgba(168,121,104,.3)", padding: "8px 12px" }}>Add</div>
              </div>
            </div>
          ))}
        </div>
      ...
      {tab === "Addresses" && (
        <div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(280px,1fr))", gap: 18 }}>
            {addresses.map((a) => (
              <div key={a.id} style={{ border: `1px solid ${a.isDefault ? "rgba(168,121,104,.5)" : "rgba(168,121,104,.16)"}`, padding: 26, background: "var(--panel)" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
                  <div style={{ fontSize: 11, letterSpacing: ".22em", textTransform: "uppercase", color: "var(--rose)" }}>{a.label}</div>
                  {a.isDefault && <div style={{ fontSize: 9.5, letterSpacing: ".18em", textTransform: "uppercase", color: "var(--bronze)", border: "1px solid rgba(168,121,104,.3)", padding: "4px 8px" }}>Default</div>}
                </div>
                <div style={{ fontSize: 13.5, lineHeight: 1.9, color: "rgba(247,241,237,.62)" }}>
                  {a.line1}<br />{a.line2 && <>{a.line2}<br /></>}{a.city}<br />{a.phone}
                </div>
                <div style={{ display: "flex", gap: 10, marginTop: 18 }}>
                  <div onClick={() => openEditAddr(a)} style={{ cursor: "pointer", fontSize: 10, letterSpacing: ".18em", textTransform: "uppercase", border: "1px solid rgba(168,121,104,.3)", padding: "8px 14px", color: "rgba(247,241,237,.7)" }}>Edit</div>
                  <div onClick={() => deleteAddr(a.id)} style={{ cursor: "pointer", fontSize: 10, letterSpacing: ".18em", textTransform: "uppercase", border: "1px solid rgba(200,90,90,.3)", padding: "8px 14px", color: "#E0A099" }}>Delete</div>
                </div>
              </div>
            ))}
            {addresses.length === 0 && <div style={{ border: "1px dashed rgba(168,121,104,.28)", display: "grid", placeItems: "center", minHeight: 150, fontSize: 11, letterSpacing: ".22em", textTransform: "uppercase", color: "rgba(247,241,237,.45)" }}>No saved addresses</div>}
          </div>
          {addrForm && (
            <div onClick={() => setAddrForm(null)} style={modalWrap}>
              <div onClick={(e) => e.stopPropagation()} style={{ background: "var(--panel)", border: "1px solid rgba(168,121,104,.3)", maxWidth: 480, width: "100%", padding: 30 }}>
      ...
      {tab === "Settings" && (
        <div style={{ maxWidth: 460, display: "flex", flexDirection: "column", gap: 32 }}>
          {/* Profile */}
          <div style={{ border: "1px solid rgba(168,121,104,.16)", background: "var(--panel)", padding: 28 }}>
          ...
          <div style={{ border: "1px solid rgba(168,121,104,.16)", background: "var(--panel)", padding: 28 }}>
            ...
            {referralCode ? (
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                <div style={{ border: "1px solid rgba(168,121,104,.25)", padding: "12px 16px", fontSize: 13, letterSpacing: ".12em", color: "var(--rose-light)", background: "rgba(168,121,104,.05)", wordBreak: "break-all" }}>
      ...
      {orderDetail && (
        <div onClick={() => setOrderDetail(null)} style={modalWrap}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: "var(--panel)", border: "1px solid rgba(168,121,104,.3)", maxWidth: 580, width: "100%", maxHeight: "90vh", overflowY: "auto", padding: 30 }}>
            ...
            <div style={{ display: "flex", gap: 10, marginBottom: 22, flexWrap: "wrap" }}>
              <span style={{ fontSize: 10, letterSpacing: ".18em", textTransform: "uppercase", border: "1px solid rgba(168,121,104,.3)", padding: "6px 11px", color: "var(--rose)" }}>{orderDetail.status}</span>
              <span style={{ fontSize: 10, letterSpacing: ".18em", textTransform: "uppercase", border: "1px solid rgba(168,121,104,.2)", padding: "6px 11px", color: "rgba(247,241,237,.55)" }}>{orderDetail.date}</span>
              <span style={{ fontSize: 10, letterSpacing: ".18em", textTransform: "uppercase", border: "1px solid rgba(168,121,104,.2)", padding: "6px 11px", color: "rgba(247,241,237,.55)" }}>{orderDetail.paymentMethod === "cod" ? "Cash on Delivery" : "JazzCash"}</span>
            </div>
            ...
            <div style={{ border: "1px solid rgba(168,121,104,.14)", marginBottom: 20 }}>
              <div style={{ display: "grid", gridTemplateColumns: "56px 1fr auto", gap: 14, padding: "12px 16px", borderBottom: "1px solid rgba(168,121,104,.14)", fontSize: 9.5, letterSpacing: ".2em", textTransform: "uppercase", color: "rgba(247,241,237,.4)" }}>
                <div></div><div>Item</div><div>Price</div>
              </div>
              {orderDetail.items.map((it, i) => (
                <div key={i} style={{ display: "grid", gridTemplateColumns: "56px 1fr auto", gap: 14, padding: "12px 16px", borderBottom: "1px solid rgba(168,121,104,.08)", alignItems: "center", fontSize: 13 }}>
              ...
              <div style={{ padding: "12px 16px 8px", display: "flex", flexDirection: "column", gap: 6, fontSize: 12.5 }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", fontFamily: "var(--serif)", fontSize: 18, color: "var(--rose-light)", paddingTop: 8, borderTop: "1px solid rgba(168,121,104,.14)", marginTop: 4 }}><span>Total</span><span>{rs(orderDetail.total)}</span></div>
              </div>
            </div>
