...
export default function ShopClient({ products, initialCat, initialSearch = "" }) {
  ...
  return (
    <div style={{ maxWidth: 1240, margin: "0 auto", padding: "44px 24px 90px" }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: 20, flexWrap: "wrap", borderBottom: "1px solid rgba(168,121,104,.14)", paddingBottom: 26, marginBottom: 34 }}>
        <div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search sets…" style={{ background: "var(--panel)", color: "var(--ink)", border: "1px solid rgba(168,121,104,.28)", padding: "11px 16px", fontSize: 12.5, outline: "none", width: 190 }} />
          <select value={sort} onChange={(e) => setSort(e.target.value)} style={{ background: "var(--panel)", color: "var(--ink)", border: "1px solid rgba(168,121,104,.28)", padding: "11px 14px", fontSize: 12.5, outline: "none" }}>
      ...
      <div data-r="sidebar" style={{ display: "grid", gridTemplateColumns: "230px 1fr", gap: 44, alignItems: "start" }}>
        <aside style={{ position: "sticky", top: 110 }}>
          ...
          {FILTER_GROUPS.map((g) => (
            <div key={g.key} style={{ borderTop: "1px solid rgba(168,121,104,.14)", padding: "20px 0" }}>
                ...
                {g.opts.map((o) => {
                  ...
                  return (
                    <div key={o} onClick={() => toggleFilter(g.key, o)} className="pill" style={{ cursor: "pointer", display: "flex", alignItems: "center", gap: 10, fontSize: 13, color: on ? "var(--rose-light)" : "rgba(247,241,237,.7)" }}>
                      <span style={{ width: 15, height: 15, border: "1px solid rgba(168,121,104,.4)", display: "grid", placeItems: "center", fontSize: 9, color: "var(--rose-light)" }}>{on ? "✓" : ""}</span>{o}
                    </div>
                  );
                })}
              </div>
          ...
          <div style={{ borderTop: "1px solid rgba(168,121,104,.14)", padding: "20px 0" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(240px,1fr))", gap: 22 }}>
            ...
            return (
              <div key={p.slug} className="card">
                <div className="zoom-wrap" style={{ position: "relative", aspectRatio: "4/5", overflow: "hidden" }}>
                  ...
                  {p.stock === 0 && <div style={{ position: "absolute", top: 14, left: 14, background: "rgba(10,10,11,.88)", border: "1px solid rgba(200,90,90,.4)", fontSize: 9.5, letterSpacing: ".2em", textTransform: "uppercase", padding: "6px 11px", color: "#E0A099" }}>Sold out</div>}
                  <div onClick={() => toggleWish(p.slug)} style={{ position: "absolute", top: 12, right: 12, width: 34, height: 34, display: "grid", placeItems: "center", borderRadius: "50%", background: "rgba(10,10,11,.75)", border: "1px solid rgba(168,121,104,.28)", color: "var(--rose)", fontSize: 15, cursor: "pointer" }}>{wished ? "♥" : "♡"}</div>
                  {p.stock > 0 && <div onClick={() => setQuick(p.slug)} style={{ position: "absolute", bottom: 12, left: 12, right: 12, textAlign: "center", background: "rgba(10,10,11,.86)", border: "1px solid rgba(168,121,104,.3)", padding: 11, fontSize: 10.5, letterSpacing: ".22em", textTransform: "uppercase", color: "var(--rose-light)", cursor: "pointer" }}>Quick view</div>}
                </div>
          ...
          {shown.length === 0 && (
            <div style={{ gridColumn: "1/-1", border: "1px dashed rgba(168,121,104,.25)", padding: 40, textAlign: "center", color: "rgba(247,241,237,.5)", fontSize: 13.5 }}>No sets match these filters. <span onClick={clearFilters} style={{ cursor: "pointer", color: "var(--rose)" }}>Clear filters</span></div>
          )}
        </div>
      </div>
      ...
      {quickP && (
        <div onClick={() => setQuick(null)} style={{ position: "fixed", inset: 0, zIndex: 120, background: "rgba(4,4,5,.85)", backdropFilter: "blur(6px)", display: "grid", placeItems: "center", padding: 24 }}>
          <div data-r="modal" onClick={(e) => e.stopPropagation()} style={{ background: "var(--panel)", border: "1px solid rgba(168,121,104,.3)", maxWidth: 820, width: "100%", display: "grid", gridTemplateColumns: "1fr 1fr" }}>
            ...
            <div style={{ padding: 36 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10, marginTop: 24 }}>
                ...
                <div onClick={() => router.push(`/product/${quickP.slug}`)} style={{ cursor: "pointer", textAlign: "center", border: "1px solid rgba(168,121,104,.3)", padding: 15, fontSize: 11, letterSpacing: ".24em", textTransform: "uppercase", color: "rgba(247,241,237,.75)" }}>Full details</div>
              </div>
            </div>
          </div>
        </div>
