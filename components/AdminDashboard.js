  ...
  return (
    <div data-r="admin" style={{ display: "grid", gridTemplateColumns: "214px 1fr", minHeight: "100vh", background: "var(--bg)" }}>
      ...
      <div style={{ padding: "34px 34px 80px" }}>
          ...
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18, flexWrap: "wrap", gap: 14 }}>
              ...
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
                ...
                <a href={`/api/admin/orders/export?status=${orderFilter}`} download style={{ fontSize: 10.5, letterSpacing: ".2em", textTransform: "uppercase", border: "1px solid rgba(227,183,166,.3)", padding: "10px 16px", color: "rgba(247,241,237,.7)" }}>Export CSV</a>
              </div>
            </div>
            <div style={{ display: "flex", gap: 10, margin: "0 0 14px", flexWrap: "wrap", alignItems: "center" }}>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                {ORDER_FILTERS.map((f) => (
                  <div key={f} onClick={() => { setOrderFilter(f); setSelectedOrders(new Set()); }} className="pill" style={{ cursor: "pointer", fontSize: 10.5, letterSpacing: ".2em", textTransform: "uppercase", padding: "9px 13px", border: `1px solid ${orderFilter === f ? "var(--bronze)" : "rgba(227,183,166,.25)"}`, color: orderFilter === f ? "var(--rose-light)" : "rgba(247,241,237,.65)", whiteSpace: "nowrap" }}>{f}</div>
                ))}
              </div>
            </div>
            <div data-scroll-x="1" style={{ border: "1px solid rgba(227,183,166,.16)", background: "var(--panel)" }}>
              ...
              {shownOrders.map((o) => (
                <div key={o.id} style={{ minWidth: 1060, display: "grid", gridTemplateColumns: "40px 110px 1.4fr 1fr 110px 160px auto", gap: 14, padding: "18px 22px", borderBottom: "1px solid rgba(227,183,166,.08)", alignItems: "start", fontSize: 12.5, background: selectedOrders.has(o.id) ? "rgba(227,183,166,.04)" : "transparent" }}>
                  ...
                  <div>{rs(o.total)}{o.paymentMethod === "cod" ? <div style={{ fontSize: 10, color: "rgba(247,241,237,.4)", marginTop: 3 }}>COD</div> : null}</div>
                  <div>
              ...
              {shownOrders.length === 0 && <div style={{ padding: 30, color: "rgba(247,241,237,.4)", fontSize: 13 }}>No orders match.</div>}
            </div>
          </div>
        )}
          ...
          <div>
            ...
            {bulkEdit ? (
              <div style={{ border: "1px solid rgba(227,183,166,.16)", background: "var(--panel)" }}>
                <div style={{ display: "grid", gridTemplateColumns: "56px 1fr 120px 110px", gap: 14, padding: "14px 22px", borderBottom: "1px solid rgba(227,183,166,.14)", fontSize: 10, letterSpacing: ".22em", textTransform: "uppercase", color: "rgba(247,241,237,.4)" }}>
                  <div></div><div>Product</div><div>Current stock</div><div>New stock</div>
                </div>
                {products.map((p) => (
                  <div key={p.id} style={{ display: "grid", gridTemplateColumns: "56px 1fr 120px 110px", gap: 14, padding: "12px 22px", borderBottom: "1px solid rgba(227,183,166,.08)", alignItems: "center", fontSize: 12.5 }}>
                    <img src={p.image} alt="" style={{ width: 40, height: 48, objectFit: "cover" }} />
                    <div><div style={{ fontFamily: "var(--serif)", fontSize: 17 }}>{p.name}</div><div style={{ fontSize: 11, color: "rgba(247,241,237,.38)" }}>{rs(p.price)}</div></div>
                    <div style={{ color: p.stock <= 3 ? "#E39B9B" : "rgba(247,241,237,.6)" }}>{p.stock} units</div>
              ...
              <div data-scroll-x="1" style={{ border: "1px solid rgba(227,183,166,.16)", background: "var(--panel)" }}>
                <div style={{ minWidth: 860, display: "grid", gridTemplateColumns: "70px 1.6fr 1fr 100px 90px 110px 140px", gap: 14, padding: "16px 22px", borderBottom: "1px solid rgba(227,183,166,.14)", fontSize: 10, letterSpacing: ".22em", textTransform: "uppercase", color: "rgba(247,241,237,.4)" }}>
                  <div></div><div>Product</div><div>Category</div><div>Price</div><div>Stock</div><div>Featured</div><div></div>
                </div>
                {products.map((p) => (
                  <div key={p.id} style={{ minWidth: 860, display: "grid", gridTemplateColumns: "70px 1.6fr 1fr 100px 90px 110px 140px", gap: 14, padding: "14px 22px", borderBottom: "1px solid rgba(227,183,166,.08)", alignItems: "center", fontSize: 12.5 }}>
                    <img src={p.image} alt={p.name} style={{ width: 48, height: 56, objectFit: "cover" }} />
                    <div><div style={{ fontFamily: "var(--serif)", fontSize: 19 }}>{p.name}</div><div style={{ fontSize: 11, color: "rgba(247,241,237,.38)", marginTop: 3 }}>{p.colorway}</div></div>
                    <div style={{ color: "rgba(247,241,237,.6)" }}>{p.categoryName}</div>
                    <div style={{ color: "var(--rose-light)" }}>{rs(p.price)}{p.wasPrice ? <span style={{ fontSize: 10, color: "rgba(247,241,237,.35)", textDecoration: "line-through", marginLeft: 6 }}>{rs(p.wasPrice)}</span> : null}</div>
                    <div style={{ color: p.stock <= 3 ? "#E39B9B" : "inherit" }}>{p.stock}{p.stock <= 3 ? " ⚠" : ""}</div>
                    <div style={{ color: "var(--rose)" }}>{p.featured ? "Featured" : "—"}</div>
                    <div style={{ display: "flex", gap: 8 }}>
                      <div onClick={() => setEditProduct(p)} style={{ cursor: "pointer", fontSize: 10, letterSpacing: ".14em", textTransform: "uppercase", border: "1px solid rgba(227,183,166,.3)", padding: "8px 11px", color: "rgba(247,241,237,.7)" }}>Edit</div>
        ...
        {tab === "Customers" && (
          <div>
            <h1 style={h1}>Customers <span style={{ fontSize: 14, color: "rgba(247,241,237,.4)", fontFamily: "var(--sans)" }}>— click a row for full order history</span></h1>
            <div data-scroll-x="1" style={{ border: "1px solid rgba(227,183,166,.16)", background: "var(--panel)" }}>
              <div style={{ minWidth: 720, display: "grid", gridTemplateColumns: "1.4fr 1fr 1fr 100px 130px 130px", gap: 14, padding: "16px 22px", borderBottom: "1px solid rgba(227,183,166,.14)", fontSize: 10, letterSpacing: ".22em", textTransform: "uppercase", color: "rgba(247,241,237,.4)" }}>
                <div>Name</div><div>Email</div><div>Phone</div><div>Orders</div><div>Spent</div><div>City</div>
              </div>
              {customers.map((c) => (
                <div key={c.id} onClick={() => setCustomerDetail(c)} style={{ minWidth: 720, display: "grid", gridTemplateColumns: "1.4fr 1fr 1fr 100px 130px 130px", gap: 14, padding: "16px 22px", borderBottom: "1px solid rgba(227,183,166,.08)", alignItems: "center", fontSize: 12.5, cursor: "pointer" }} className="admin-tab">
                  <div style={{ fontFamily: "var(--serif)", fontSize: 17 }}>{c.name}</div>
                  <div style={{ color: "rgba(247,241,237,.55)", fontSize: 11.5 }}>{c.email}</div>
                  <div style={{ color: "rgba(247,241,237,.55)" }}>{c.phone}</div>
                  <div>{c.orders}</div>
                  <div style={{ color: "var(--rose-light)" }}>{rs(c.spent)}</div>
                  <div style={{ color: "rgba(247,241,237,.55)" }}>{c.city}</div>
                </div>
              ))}
              {customers.length === 0 && <div style={{ padding: 30, color: "rgba(247,241,237,.4)", fontSize: 13 }}>No customers yet.</div>}
            </div>
            {customerDetail && (
              <div onClick={() => setCustomerDetail(null)} style={modalWrap}>
                <div onClick={(e) => e.stopPropagation()} style={{ background: "var(--panel)", border: "1px solid rgba(227,183,166,.3)", maxWidth: 640, width: "100%", maxHeight: "88vh", overflowY: "auto", padding: 30 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18 }}>
                    <div style={{ fontFamily: "var(--serif)", fontSize: 28 }}>{customerDetail.name}</div>
                    <div onClick={() => setCustomerDetail(null)} style={{ cursor: "pointer", fontSize: 22, color: "rgba(247,241,237,.5)" }}>×</div>
                  </div>
                  <div style={{ fontSize: 12.5, color: "rgba(247,241,237,.5)", marginBottom: 6 }}>{customerDetail.email} · {customerDetail.phone} · {customerDetail.city}</div>
                  ...
                  <div style={{ border: "1px solid rgba(227,183,166,.16)" }}>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 100px 130px", gap: 12, padding: "10px 18px", borderBottom: "1px solid rgba(227,183,166,.14)", fontSize: 9.5, letterSpacing: ".22em", textTransform: "uppercase", color: "rgba(247,241,237,.4)" }}>
                      <div>Order</div><div>Status</div><div>Total</div>
                    </div>
                    {(customerDetail.orderList || []).map((o) => (
                      <div key={o.code} style={{ display: "grid", gridTemplateColumns: "1fr 100px 130px", gap: 12, padding: "12px 18px", borderBottom: "1px solid rgba(227,183,166,.08)", alignItems: "center", fontSize: 12.5 }}>
                        <div><span style={{ color: "var(--rose-light)" }}>{o.code}</span><span style={{ color: "rgba(247,241,237,.4)", fontSize: 11, marginLeft: 10 }}>{o.date}</span></div>
                        <div><span style={statusBadge(o.status)}>{o.status}</span></div>
                        <div>{rs(o.total)}</div>
                      </div>
                    ))}
                    {(customerDetail.orderList || []).length === 0 && <div style={{ padding: 18, color: "rgba(247,241,237,.4)", fontSize: 13 }}>No orders.</div>}
                  </div>
                </div>
              </div>
            )}
      ...
      {shot && (
        <div onClick={() => setShot(null)} style={modalWrap}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: "var(--panel)", border: "1px solid rgba(227,183,166,.3)", maxWidth: 500, width: "100%", maxHeight: "92vh", overflowY: "auto", padding: 30 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18 }}>
              ...
              <div onClick={() => setShot(null)} style={{ cursor: "pointer", fontSize: 20, color: "rgba(247,241,237,.5)" }}>×</div>
            </div>
            {shot.paymentProof ? (
              <img src={shot.paymentProof} alt="Payment proof" style={{ width: "100%", objectFit: "contain", background: "#000", display: "block" }} />
            ) : (
              <div style={{ padding: 30, textAlign: "center", color: "rgba(247,241,237,.5)", border: "1px dashed rgba(227,183,166,.25)", fontSize: 13 }}>{shot.paymentMethod === "cod" ? "Cash on delivery — no screenshot." : "No screenshot uploaded."}</div>
            )}
...
function Overview({ kpis, orders, products }) {
  ...
  return (
    <div>
      ...
      <div style={{ fontSize: 12, color: "rgba(247,241,237,.42)", marginBottom: 26 }}>Live from your store</div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: 14, marginBottom: 20 }}>
        {kpiCards.map((k) => (
          <div key={k.label} style={{ border: "1px solid rgba(227,183,166,.16)", background: "var(--panel)", padding: 20 }}>
            <div style={{ fontSize: 9.5, letterSpacing: ".24em", textTransform: "uppercase", color: "rgba(247,241,237,.42)" }}>{k.label}</div>
      ...
      <div data-r="split" style={{ display: "grid", gridTemplateColumns: "1.6fr 1fr", gap: 18, alignItems: "start" }}>
        <div style={{ border: "1px solid rgba(227,183,166,.16)", background: "var(--panel)", padding: 26 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 22 }}>
            ...
            <div style={{ fontSize: 11.5, color: "rgba(247,241,237,.42)" }}>{rs(chart.reduce((s, d) => s + d.total, 0))} total</div>
          </div>
          ...
          {maxVal === 1 && <div style={{ textAlign: "center", fontSize: 12, color: "rgba(247,241,237,.35)", marginTop: 10 }}>No revenue data yet — orders will appear here</div>}
        </div>
        <div style={{ border: "1px solid rgba(227,183,166,.16)", background: "var(--panel)", padding: 26 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            {alerts.map((a, i) => (
              <div key={i} style={{ display: "flex", gap: 12, alignItems: "flex-start", borderBottom: "1px solid rgba(227,183,166,.1)", paddingBottom: 14 }}>
                <span style={{ width: 7, height: 7, borderRadius: "50%", background: a.dot, marginTop: 5, flexShrink: 0 }} />
                <div><div style={{ fontSize: 13, color: "rgba(247,241,237,.8)" }}>{a.title}</div><div style={{ fontSize: 11.5, color: "rgba(247,241,237,.4)", marginTop: 4 }}>{a.meta}</div></div>
              </div>
            ))}
          </div>
        </div>
  ...
  return (
    <div data-r="split" style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 22, alignItems: "start" }}>
      <div>
        ...
        <div style={{ border: "1px solid rgba(227,183,166,.16)", background: "var(--panel)" }}>
          {categories.map((c) => (
            <div key={c.id} style={{ display: "grid", gridTemplateColumns: "56px 1fr auto", gap: 14, padding: "16px 22px", borderBottom: "1px solid rgba(227,183,166,.08)", alignItems: "center" }}>
              <img src={c.image} alt={c.name} style={{ width: 50, height: 60, objectFit: "cover" }} />
              <div>
                <div style={{ fontFamily: "var(--serif)", fontSize: 19 }}>{c.name}</div>
                <div style={{ fontSize: 11, color: "rgba(247,241,237,.38)", marginTop: 3 }}>/{c.slug}</div>
              </div>
              <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                <span onClick={() => call(`/api/admin/categories/${c.id}`, "PATCH", { showOnHome: !c.showOnHome })} style={{ cursor: "pointer", fontSize: 10, letterSpacing: ".14em", textTransform: "uppercase", border: "1px solid rgba(227,183,166,.28)", padding: "7px 10px", color: c.showOnHome ? "#8FD6A6" : "rgba(247,241,237,.4)" }}>{c.showOnHome ? "Shown" : "Hidden"}</span>
                <div onClick={() => setEditCat({ ...c })} style={{ cursor: "pointer", fontSize: 10, letterSpacing: ".14em", textTransform: "uppercase", border: "1px solid rgba(227,183,166,.3)", padding: "7px 10px", color: "rgba(247,241,237,.7)" }}>Edit</div>
          ...
          {categories.length === 0 && <div style={{ padding: 30, color: "rgba(247,241,237,.4)", fontSize: 13 }}>No categories yet.</div>}
        </div>
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
        ...
        <div style={{ border: "1px solid rgba(227,183,166,.14)", background: "var(--panel)", padding: 18, fontSize: 12, color: "rgba(247,241,237,.45)", lineHeight: 1.7 }}>
...
function CategoryEditor({ cat, onClose, onSaved, uploading, setUploading }) {
  ...
  return (
    <div onClick={onClose} style={modalWrap}>
      <div onClick={(e) => e.stopPropagation()} style={{ background: "var(--panel)", border: "1px solid rgba(227,183,166,.3)", maxWidth: 480, width: "100%", padding: 28 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18 }}>
          <div style={{ fontFamily: "var(--serif)", fontSize: 24 }}>Edit category</div>
          <div onClick={onClose} style={{ cursor: "pointer", fontSize: 22, color: "rgba(247,241,237,.5)" }}>×</div>
        </div>
        <div style={{ display: "flex", gap: 14, marginBottom: 16 }}>
          <img src={form.image} alt="" style={{ width: 72, height: 88, objectFit: "cover", border: "1px solid rgba(227,183,166,.3)" }} />
          <div style={{ flex: 1 }}>
            <label style={{ cursor: "pointer", display: "block", border: "1px dashed rgba(227,183,166,.4)", padding: 12, textAlign: "center", fontSize: 11, letterSpacing: ".14em", textTransform: "uppercase", color: "rgba(247,241,237,.6)" }}>
              {uploading ? "Uploading…" : "Upload photo"}
...
function Homepage({ content, call, busy, waTpls, setWaTpls }) {
  ...
  return (
    <div style={{ maxWidth: 860 }}>
      ...
      <div style={{ fontSize: 12, color: "rgba(247,241,237,.42)", marginBottom: 28 }}>Edit the live storefront without touching code.</div>

      {/* Hero */}
      <div style={{ border: "1px solid rgba(227,183,166,.16)", background: "var(--panel)", padding: 28, marginBottom: 20 }}>
      ...
      <div style={{ border: "1px solid rgba(227,183,166,.16)", background: "var(--panel)", padding: 28, marginBottom: 20 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1.6fr", gap: 20, alignItems: "start" }}>
          <div>
            <img src={form.studioImage} alt="" style={{ width: "100%", aspectRatio: "4/5", objectFit: "cover", border: "1px solid rgba(227,183,166,.3)", display: "block", marginBottom: 10 }} />
            <label style={{ cursor: "pointer", display: "block", border: "1px dashed rgba(227,183,166,.4)", padding: "10px 14px", textAlign: "center", fontSize: 10.5, letterSpacing: ".14em", textTransform: "uppercase", color: "rgba(247,241,237,.6)", marginBottom: 8 }}>
      ...
      <div style={{ border: "1px solid rgba(227,183,166,.16)", background: "var(--panel)", padding: 28, marginBottom: 20 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1.6fr", gap: 20, alignItems: "start" }}>
          <div>
            <img src={form.aboutImage1} alt="" style={{ width: "100%", aspectRatio: "4/5", objectFit: "cover", border: "1px solid rgba(227,183,166,.3)", display: "block", marginBottom: 10 }} />
            <label style={{ cursor: "pointer", display: "block", border: "1px dashed rgba(227,183,166,.4)", padding: "10px 14px", textAlign: "center", fontSize: 10.5, letterSpacing: ".14em", textTransform: "uppercase", color: "rgba(247,241,237,.6)", marginBottom: 8 }}>
              {uploading ? "Uploading…" : "Upload new photo"}
      ...
      <div style={{ border: "1px solid rgba(227,183,166,.16)", background: "var(--panel)", padding: 28, marginBottom: 20 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1.6fr", gap: 20, alignItems: "start" }}>
          <div>
            <img src={form.aboutArtistImg} alt="" style={{ width: "100%", aspectRatio: "4/5", objectFit: "cover", border: "1px solid rgba(227,183,166,.3)", display: "block", marginBottom: 10 }} />
            <label style={{ cursor: "pointer", display: "block", border: "1px dashed rgba(227,183,166,.4)", padding: "10px 14px", textAlign: "center", fontSize: 10.5, letterSpacing: ".14em", textTransform: "uppercase", color: "rgba(247,241,237,.6)", marginBottom: 8 }}>
              {uploading ? "Uploading…" : "Upload new photo"}
      ...
      <div style={{ border: "1px solid rgba(227,183,166,.16)", background: "var(--panel)", padding: 28, marginBottom: 20 }}>
        ...
        <div style={{ fontSize: 12, color: "rgba(247,241,237,.4)", marginBottom: 20, lineHeight: 1.7 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div style={{ fontSize: 11.5, color: "rgba(247,241,237,.4)", lineHeight: 1.7 }}>
      ...
      <div style={{ border: "1px solid rgba(227,183,166,.16)", background: "var(--panel)", padding: 28, marginBottom: 20 }}>
        ...
        <div style={{ fontSize: 12, color: "rgba(247,241,237,.4)", marginBottom: 20 }}>Use <strong style={{ color: "var(--rose-light)" }}>{"{name}"}</strong> for customer name and <strong style={{ color: "var(--rose-light)" }}>{"{code}"}</strong> for order number.</div>
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          {[
    ...
    <div onClick={onClose} style={modalWrap}>
      <div onClick={(e) => e.stopPropagation()} style={{ background: "var(--panel)", border: "1px solid rgba(227,183,166,.3)", maxWidth: 640, width: "100%", maxHeight: "88vh", overflowY: "auto", padding: 30 }}>
        ...
        <div style={{ display: "flex", gap: 16, marginBottom: 16 }}>
          <img src={form.image} alt="" style={{ width: 90, height: 108, objectFit: "cover", border: "1px solid rgba(227,183,166,.3)" }} />
          <div style={{ flex: 1 }}>
            <label style={{ cursor: "pointer", display: "block", border: "1px dashed rgba(227,183,166,.4)", padding: 14, textAlign: "center", fontSize: 11, letterSpacing: ".16em", textTransform: "uppercase", color: "rgba(247,241,237,.7)" }}>
              {uploading ? "Uploading…" : "Upload photo"}
              <input type="file" accept="image/*" onChange={upload} style={{ display: "none" }} />
            </label>
        ...
        <label style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 14, fontSize: 12.5, color: "rgba(247,241,237,.7)", cursor: "pointer" }}>
...
function MessagesTab({ messages, subscribers }) {
  ...
  return (
    <div>
      ...
      {view === "messages" && (
        <div style={{ border: "1px solid rgba(227,183,166,.16)", background: "var(--panel)" }}>
          {messages.length === 0 && <div style={{ padding: 30, color: "rgba(247,241,237,.4)", fontSize: 13 }}>No messages yet.</div>}
          {messages.map((m) => (
            <div key={m.id} style={{ padding: "20px 24px", borderBottom: "1px solid rgba(227,183,166,.08)" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 16, flexWrap: "wrap" }}>
                <div>
                  <div style={{ fontFamily: "var(--serif)", fontSize: 20, marginBottom: 4 }}>{m.name}</div>
                  <div style={{ fontSize: 11.5, color: "rgba(247,241,237,.4)" }}>{m.phone || "No phone"}{m.topic ? " · " + m.topic : ""} · {m.date}</div>
                </div>
                {m.phone && (
              ...
              <p style={{ fontSize: 13.5, lineHeight: 1.8, color: "rgba(247,241,237,.65)", margin: "12px 0 0", maxWidth: 680 }}>{m.body}</p>
            </div>
          ))}
        </div>
      )}
      ...
      {view === "subscribers" && (
        <div style={{ border: "1px solid rgba(227,183,166,.16)", background: "var(--panel)" }}>
          <div style={{ padding: "14px 24px", borderBottom: "1px solid rgba(227,183,166,.14)", fontSize: 10, letterSpacing: ".22em", textTransform: "uppercase", color: "rgba(247,241,237,.4)", display: "grid", gridTemplateColumns: "1fr auto" }}>
            <div>Email</div><div>Joined</div>
          </div>
          {subscribers.length === 0 && <div style={{ padding: 30, color: "rgba(247,241,237,.4)", fontSize: 13 }}>No subscribers yet.</div>}
          {subscribers.map((s) => (
            <div key={s.id} style={{ padding: "14px 24px", borderBottom: "1px solid rgba(227,183,166,.08)", display: "grid", gridTemplateColumns: "1fr auto", alignItems: "center", fontSize: 13 }}>
              <div style={{ color: "var(--rose-light)" }}>{s.email}</div>
              <div style={{ fontSize: 11.5, color: "rgba(247,241,237,.4)" }}>{s.date}</div>
            </div>
          ))}
        </div>
      )}
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
        <h1 style={{ ...h1, marginBottom: 0 }}>Reviews <span style={{ fontSize: 14, color: "rgba(247,241,237,.4)", fontFamily: "var(--sans)" }}>({localReviews.length})</span></h1>
      </div>
      <div style={{ border: "1px solid rgba(227,183,166,.16)", background: "var(--panel)" }}>
        {localReviews.length === 0 && <div style={{ padding: 30, color: "rgba(247,241,237,.4)", fontSize: 13 }}>No reviews yet.</div>}
        {localReviews.map((r) => (
          <div key={r.id} style={{ padding: "20px 24px", borderBottom: "1px solid rgba(227,183,166,.08)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 16, flexWrap: "wrap" }}>
              <div style={{ flex: 1 }}>
                <div style={{ display: "flex", gap: 12, alignItems: "center", marginBottom: 6 }}>
                  ...
                  <span style={{ fontSize: 11, color: "rgba(247,241,237,.35)" }}>{r.date}</span>
                </div>
                ...
                <p style={{ fontSize: 13, lineHeight: 1.8, color: "rgba(247,241,237,.65)", margin: 0 }}>{r.body}</p>
              </div>
              <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
                <div onClick={() => toggleVerified(r)} style={{ cursor: "pointer", fontSize: 10, letterSpacing: ".14em", textTransform: "uppercase", border: `1px solid ${r.verified ? "rgba(143,214,166,.5)" : "rgba(227,183,166,.3)"}`, padding: "7px 11px", color: r.verified ? "#8FD6A6" : "rgba(247,241,237,.6)" }}>{r.verified ? "Verified ✓" : "Mark verified"}</div>
...
function SettingsTab({ content, call, busy }) {
  ...
  return (
    <div style={{ maxWidth: 680 }}>
      ...
      <div style={{ fontSize: 12, color: "rgba(247,241,237,.42)", marginBottom: 28 }}>Changes take effect immediately on the storefront.</div>
      ...
      <div style={{ border: "1px solid rgba(227,183,166,.16)", background: "var(--panel)", padding: 28, marginBottom: 18 }}>
        ...
        <div>
          <div style={{ fontSize: 10, letterSpacing: ".2em", textTransform: "uppercase", color: "rgba(247,241,237,.4)", marginBottom: 8 }}>Closed message (shown to visitors)</div>
      ...
      <div style={{ border: "1px solid rgba(227,183,166,.16)", background: "var(--panel)", padding: 28, marginBottom: 18 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 14 }}>
          <div>
            <div style={{ fontSize: 10, letterSpacing: ".2em", textTransform: "uppercase", color: "rgba(247,241,237,.4)", marginBottom: 8 }}>Delivery fee (Rs)</div>
            <input type="number" value={form.deliveryFee} onChange={set("deliveryFee")} style={adminInput} />
          </div>
          <div>
            <div style={{ fontSize: 10, letterSpacing: ".2em", textTransform: "uppercase", color: "rgba(247,241,237,.4)", marginBottom: 8 }}>Free delivery over (Rs)</div>
            <input type="number" value={form.freeDeliveryOver} onChange={set("freeDeliveryOver")} style={adminInput} />
          </div>
          <div>
            <div style={{ fontSize: 10, letterSpacing: ".2em", textTransform: "uppercase", color: "rgba(247,241,237,.4)", marginBottom: 8 }}>COD handling fee (Rs)</div>
            <input type="number" value={form.codHandling} onChange={set("codHandling")} style={adminInput} />
          </div>
        </div>
      </div>

      <div style={{ border: "1px solid rgba(227,183,166,.16)", background: "var(--panel)", padding: 28, marginBottom: 24 }}>
...
function MediaTab() {
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
        <h1 style={{ ...h1, marginBottom: 0 }}>Media <span style={{ fontSize: 14, color: "rgba(247,241,237,.4)", fontFamily: "var(--sans)" }}>({files.length} files)</span></h1>
        <div onClick={load} style={{ cursor: "pointer", fontSize: 10.5, letterSpacing: ".2em", textTransform: "uppercase", border: "1px solid rgba(227,183,166,.3)", padding: "9px 16px", color: "rgba(247,241,237,.7)" }}>Refresh</div>
      </div>
      {files.length === 0 ? (
        <div style={{ border: "1px dashed rgba(227,183,166,.25)", padding: 40, textAlign: "center", color: "rgba(247,241,237,.5)", fontSize: 13 }}>No uploaded files yet. Images appear here when you upload via product or category editors.</div>
      ) : (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(160px,1fr))", gap: 14 }}>
          {files.map((f) => (
            <div key={f.name} style={{ border: "1px solid rgba(227,183,166,.16)", background: "var(--panel)", overflow: "hidden" }}>
              <div style={{ aspectRatio: "1/1", overflow: "hidden", position: "relative" }}>
                <img src={f.path} alt={f.name} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
              </div>
              <div style={{ padding: "10px 12px" }}>
                <div style={{ fontSize: 10.5, color: "rgba(247,241,237,.5)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", marginBottom: 6 }}>{f.name}</div>
                <div style={{ fontSize: 10, color: "rgba(247,241,237,.3)", marginBottom: 10 }}>{(f.size / 1024).toFixed(0)} KB</div>
