...
const FROM = () => `Press-Ons by Myra <${process.env.GMAIL_USER}>`;

const base = `background:#0a0a0b;color:#f7f1ed;font-family:Georgia,serif;max-width:560px;margin:0 auto;padding:40px 32px`;
const footer = `<hr style="border:none;border-top:1px solid #1e1e1e;margin:32px 0"/><p style="font-size:12px;color:rgba(247,241,237,.3)">Press-Ons by Myra · Lahore, Pakistan</p>`;
const trackLink = (code) =>
  `<p style="font-size:13px;color:rgba(247,241,237,.55);line-height:1.8;margin-top:20px">Track your order at <a href="https://pressonsbymyra.pk/track" style="color:#d4a89a">pressonsbymyra.pk/track</a> — use code <strong>${code}</strong> and your phone number. (Check spam if not in inbox)</p>`;
...
export async function sendOrderConfirmEmail(order, items, customerEmail) {
  ...
  await send(
    customerEmail,
    `Order confirmed — ${order.code} · Press-Ons by Myra`,
    `<div style="${base}">
...
export async function sendOrderStatusEmail(order, items, customerEmail) {
  ...
  if (order.status === "Confirmed") {
    ...
    subject = `Your Press-Ons by Myra order ${order.code} has been shipped! 🚚`;
    ...
    html = `<div style="${base}">
      ...
      <p style="color:rgba(247,241,237,.6);font-size:14px;line-height:1.8;margin:0 0 16px">Hi ${order.customerName}, your Press-Ons by Myra order <strong style="color:#d4a89a">${order.code}</strong> has been delivered. We hope you love your new nails!</p>
      <p style="font-size:13px;color:rgba(247,241,237,.55);line-height:1.8">Leave a review or reorder at <a href="https://pressonsbymyra.pk" style="color:#d4a89a">pressonsbymyra.pk</a></p>
      ${footer}</div>`;
  } else {
    return;
  }
...
export async function sendStockAlertEmail(productName, productSlug, email) {
  return send(
    email,
    `${productName} is back in stock! — Press-Ons by Myra`,
    `<div style="${base}">
      ...
      <a href="https://pressonsbymyra.pk/product/${productSlug}" style="display:inline-block;background:linear-gradient(100deg,#A87968,#C0907C);color:#fff;padding:14px 28px;text-decoration:none;font-size:13px;letter-spacing:.1em;text-transform:uppercase">Shop now →</a>
...
export async function sendWelcomeEmail(name, email, referralCode, welcomePoints) {
  ...
  return send(
    email,
    "Welcome to Press-Ons by Myra 🌸",
    `<div style="${base}">
      <h1 style="font-size:28px;font-weight:300;margin:0 0 8px">Welcome, ${firstName}! 🌸</h1>
      <p style="color:rgba(247,241,237,.6);font-size:14px;line-height:1.8;margin:0 0 22px">Thank you for joining Press-Ons by Myra — hand-painted press-on sets, sized to your nails and shipped nationwide from Lahore.</p>
      ${pointsLine}
      ...
      <a href="https://pressonsbymyra.pk/shop" style="display:inline-block;background:linear-gradient(100deg,#A87968,#C0907C);color:#fff;padding:14px 28px;text-decoration:none;font-size:13px;letter-spacing:.1em;text-transform:uppercase;margin-top:8px">Shop the sets →</a>
      ${footer}</div>`
  );
}
...
export async function sendAdminOrderEmail(order, items) {
  ...
  return send(
    ...
    `<div style="${base}">
      ...
      <a href="https://pressonsbymyra.pk/admin" style="display:inline-block;background:linear-gradient(100deg,#A87968,#C0907C);color:#fff;padding:13px 26px;text-decoration:none;font-size:12px;letter-spacing:.1em;text-transform:uppercase">Open admin panel →</a>
      ${footer}</div>`
  );
}
...
export async function sendGiftCardEmail(giftCard, buyerEmail) {
  ...
  return send(
    buyerEmail,
    `Your Press-Ons by Myra gift card — ${giftCard.code} 🎁`,
    `<div style="${base}">
      <h1 style="font-size:26px;font-weight:300;margin:0 0 8px">Your gift card is ready 🎁</h1>
      <p style="color:rgba(247,241,237,.6);font-size:14px;line-height:1.8;margin:0 0 20px">Here's the Press-Ons by Myra gift card${forLine}. The code below can be applied at checkout — no expiry.</p>
      ...
      <a href="https://pressonsbymyra.pk/shop" style="display:inline-block;background:linear-gradient(100deg,#A87968,#C0907C);color:#fff;padding:14px 28px;text-decoration:none;font-size:13px;letter-spacing:.1em;text-transform:uppercase;margin-top:8px">Shop now →</a>
      ${footer}</div>`
  );
}
...
export async function sendAbandonedCartEmail(name, email, items, subtotal) {
  ...
  return send(
    ...
    `<div style="${base}">
      ...
      <a href="https://pressonsbymyra.pk/cart" style="display:inline-block;background:linear-gradient(100deg,#A87968,#C0907C);color:#fff;padding:14px 30px;text-decoration:none;font-size:13px;letter-spacing:.1em;text-transform:uppercase">Complete my order →</a>
...
export async function sendPasswordResetEmail(customerName, email, link) {
  return send(
    email,
    "Reset your Press-Ons by Myra password",
    `<div style="${base}">
      ...
      <a href="${link}" style="display:inline-block;background:linear-gradient(100deg,#A87968,#C0907C);color:#fff;padding:14px 28px;text-decoration:none;font-size:13px;letter-spacing:.1em;text-transform:uppercase">Reset password</a>
