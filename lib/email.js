...
const trackLink = (code) =>
  `<p style="font-size:13px;color:rgba(247,241,237,.55);line-height:1.8;margin-top:20px">Track your order at <a href="https://mysonails.pk/track" style="color:#d4a89a">mysonails.pk/track</a> — use code <strong>${code}</strong> and your phone number.</p>`;
...
export async function sendOrderStatusEmail(order, items, customerEmail) {
  ...
  await t.sendMail({ from: FROM(), to: customerEmail, subject, html }).catch(() => {});
}
...
export async function sendPasswordResetEmail(customerName, email, link) {
  ...
  try {
    ...
    console.error("sendPasswordResetEmail SMTP error:", e.message);
    throw e;
  }
}
